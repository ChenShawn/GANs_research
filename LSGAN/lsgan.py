...
class LSGAN(object):
    ...
    # Note: input_shape must be list, not tuple or sth else
    def __init__(self, input_shape, batch_size=64, z_dim=128, g_filter_num=64, d_filter_num=64,
                 g_fc_dim=1024, d_fc_dim=1024, learning_rate=0.0002, beta1=0.5, pre_train=True,
                 model_path='./lsgan/checkpoint/', log_dir='.\\lsgan\\logs'):
        # copy parameters
    ...
    def _loss_function(self):
        self.d_loss_real = tf.reduce_sum(tf.square(self.Discriminator - 1))
        self.d_loss_fake = tf.reduce_sum((tf.square(self.Discriminator_ + 1)))
        d_loss = self.d_loss_real + self.d_loss_fake
        g_loss = tf.reduce_sum(tf.square(self.Discriminator_))
        return g_loss, d_loss

    def train(self, reader, loop=20000, print_iteration=100, critic_loop=1,
              save_iteration=500, auto_save_img=True, auto_save_model=True):
        ...
        for it in range(loop):
            ...
            if auto_save_img and it % save_iteration == 1:
                ...
                save_image(samples, name='LSGAN.png', idx=self.counter, path='./lsgan/generated/')
    ...
    def save(self):
        ...
        save_path = self.saver.save(self.sess, self.model_path + 'lsgan.model', global_step=self.counter)
...
if __name__ == '__main__':
    ...
    lsgan = LSGAN(input_shape=[28, 28, 1], pre_train=True)
    show_all_variables()
    lsgan.train(mnist.train, loop=8000, critic_loop=1,
                save_iteration=1000, auto_save_img=True, auto_save_model=False)

    batch_z = np.random.uniform(-1.0, 1.0, size=(lsgan.batch_size, lsgan.z_dim)).astype(np.float32)
    arr = lsgan.sess.run(lsgan.Sampler, feed_dict={lsgan.z_latent: batch_z})
    save_image(arr, name='test.png', idx=666, path='./lsgan/generated/')
